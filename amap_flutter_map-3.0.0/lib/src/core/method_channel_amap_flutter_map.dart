...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  Stream<MapRouteSearchDoneEvent> onRouteSearchDone({required int mapId}) {
    return _events(mapId).whereType<MapRouteSearchDoneEvent>();
  }
  ...
  Future<dynamic> _handleMethodCall(MethodCall call, int mapId) async {
    switch (call.method) {
      ...
      case 'map#onPOISearchDone':
        ...
        break;
      case 'map#onRouteSearchDone':
        try {
          _mapEventStreamController.add(
              MapRouteSearchDoneEvent(mapId, call.arguments['route']));
        } catch (e) {
          print('map#onRouteSearchDone error===>' + e.toString());
        }
        break;
    }
  }
  ...
  Future<dynamic> searchRoute({
    required int mapId
  }) {
    return channel(mapId)
        .invokeMethod<dynamic>('map#searchRoute', <String, dynamic>{
    });
  }
