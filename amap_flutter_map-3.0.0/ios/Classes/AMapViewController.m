...
- (void)setMethodCallHandler {
   ...
   [self.channel addMethodName:@"map#searchRoute" withHandler:^(FlutterMethodCall * _Nonnull call, FlutterResult  _Nonnull result) {
        NSString *approvalNumber = [weakSelf.mapView mapContentApprovalNumber];
        AMapWalkingRouteSearchRequest *navi = [[AMapWalkingRouteSearchRequest alloc] init];

        // set default latitude&longitude here
        navi.origin = [AMapGeoPoint locationWithLatitude:39.910267
                                               longitude:116.370888];
        navi.destination = [AMapGeoPoint locationWithLatitude:39.910267
...
- (void)onRouteSearchDone:(AMapRouteSearchBaseRequest *)request response:(AMapRouteSearchResponse *)response
{
    ...
    NSMutableArray *steps = [NSMutableArray arrayWithCapacity:path.steps.count];

    [response.route.paths[0].steps enumerateObjectsUsingBlock:^(AMapStep *obj, NSUInteger idx, BOOL *stop) {
          NSString *string  = [NSString stringWithFormat:@"%@\n", obj.instruction];
          [jsonString appendString:string];
     }];

    [_channel invokeMethod:@"map#onRouteSearchDone" arguments:@{@"route":jsonString}];
}
